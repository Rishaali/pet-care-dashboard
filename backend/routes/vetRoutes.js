...
router.get("/recommendations", (req, res) => {
    ...
    db.all(sql, [city], (err, rows) => {
        ...
        const scoredVets = rows.map(vet => {
            ...
            // 3. Rating = 15 points
            // Scaling rating: (rating / 5) * 15 => rating * 3
...
router.get("/:id/availability", (req, res) => {
    ...
    // 1. Fetch scheduled shifts for the vet on this date
    const availSql = `
    ...
    db.all(availSql, [vetId, date], (err, shifts) => {
        ...
        // 2. Fetch existing booked appointments for this vet on this date
        const bookedSql = `
        ...
        db.all(bookedSql, [vetId, date], (err, bookings) => {
            ...
            // 3. Generate slots (30-minute intervals) for each shift
            const slots = [];
