...
db.serialize(() => {
    ...
    `, (err) => {
        if (err) console.error("Error creating medication_logs table:", err.message);
            ...
            // After core tables are ready, create vet tables, which then add extended columns and seed
            createVetTables();
            createSuppliesTable();
            createVaccinationsTable();
            createGroomingTable();
            createVetVisitsTable();
...
function createSuppliesTable() {
    ...
    `, (err) => {
        if (err) {
            console.error("Error creating supplies table:", err.message);
        } else {
            addSuppliesColumns();
        }
    });
...
function addSuppliesColumns() {
    const columns = [
        "unit TEXT",
        "current_stock REAL NOT NULL DEFAULT 0",
        "usage_per_log REAL NOT NULL DEFAULT 0",
        "low_stock_threshold_days INTEGER DEFAULT 2",
        "last_updated DATETIME DEFAULT CURRENT_TIMESTAMP"
    ];
    columns.forEach(colDef => {
        const colName = colDef.split(" ")[0];
        db.run(`ALTER TABLE supplies ADD COLUMN ${colDef}`, (err) => {
            if (err && !err.message.includes("duplicate column")) {
                console.error(`Error adding column ${colName} to supplies:`, err.message);
            }
        });
    });
}
...
function addExtendedColumns() {
    ...
    newColumns.forEach(colDef => {
        ...
        db.run(`ALTER TABLE pets ADD COLUMN ${colDef}`, (err) => {
            ...
            pending--;
            if (pending === 0) {
                // Also ensure medications table has status, snoozed_until, completed_at columns
                const medCols = [
                ...
                medCols.forEach(mDef => {
                    db.run(`ALTER TABLE medications ADD COLUMN ${mDef}`, () => {});
                });
                seedDatabase();
            }
        });
...
function createVaccinationsTable() {
    db.run(`
    ...
    `, (err) => {
        if (err) {
            console.error("Error creating vaccinations table:", err.message);
        }
    });
}

function createGroomingTable() {
    db.run(`
    ...
    `, (err) => {
        if (err) {
            console.error("Error creating grooming table:", err.message);
        }
    });
}

function createVetVisitsTable() {
    db.run(`
    ...
    `, (err) => {
        if (err) {
            console.error("Error creating vet_visits table:", err.message);
        }
    });
}
...
function createNotificationsTables() {
    ...
    `, (err) => {
        if (err) {
            ...
            db.run(`CREATE INDEX IF NOT EXISTS idx_notifications_key ON notifications(reminder_key)`);
        }
    });
    ...
    db.run(`
        CREATE TABLE IF NOT EXISTS notification_preferences (
            ...
            timing_preference TEXT DEFAULT '24h,1h',
