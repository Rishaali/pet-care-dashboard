            ...
            db.get("SELECT u.name AS user_name, u.email AS user_email, p.name AS pet_name FROM users u JOIN pets p ON p.user_id = u.id WHERE u.id = ? AND p.id = ?", [req.user.id, pet_id], (uErr, row) => {
                if (!uErr && row) {
                    dispatchNotification({
                        ...
                        type: 'medication',
                        referenceId: medId,
                        reminderKey: `med_scheduled_${medId}`,
                        title: `Medication Scheduled: ${medication_name.trim()} for ${row.pet_name}`,
                        message: `Medication schedule for ${row.pet_name} (${medication_name.trim()} - ${dosage.trim()}) has been set with daily reminder at ${reminder_time.trim()}.`,
                        actionUrl: `/index.html#medications`,
                        userEmail: row.user_email,
                        userName: row.user_name,
                        details: [
                            { label: "Pet", value: row.pet_name },
                            { label: "Medication", value: medication_name.trim() },
                            { label: "Dosage", value: dosage.trim() },
                            { label: "Daily Time", value: reminder_time.trim() },
                            { label: "Frequency", value: frequency ? frequency.trim() : "Daily" }
                        ],
                        actionText: "View Medication Schedules"
                    }).catch(e => console.error("Error dispatching medication schedule notification:", e));
                }
            });
...
medicationRouter.put("/:id", authMiddleware, audioUpload.single("audio_file"), (req, res) => {
    ...
    db.get("SELECT id, user_id FROM pets WHERE id = ?", [pet_id], (err, petRow) => {
        ...
        db.get(checkSql, [id, req.user.id], (err, existingMed) => {
            ...
            const sql = `
                UPDATE medications
                SET pet_id = ?, medication_name = ?, dosage = ?, frequency = ?, start_date = ?, end_date = ?, reminder_time = ?, notes = ?, sound_type = ?, custom_sound = ?, status = 'PENDING', snoozed_until = NULL, completed_at = NULL
                WHERE id = ?
            `;
            const params = [
                pet_id,
