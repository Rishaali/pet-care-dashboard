...
router.get("/recommendations", authMiddleware, (req, res) => {
    ...
    db.all(sql, [city], (err, rows) => {
        ...
        const scoredVets = rows.map(vet => {
            ...
            const hasAvailability = vet.availability_count > 0 || (vet.clinic_id !== null && vet.clinic_id !== undefined);
            if (hasAvailability) {
                score += 10;
...
router.get("/:id/availability", authMiddleware, (req, res) => {
    ...
    // 1. Verify vet exists
    db.get("SELECT v.*, c.opening_time, c.closing_time FROM vets v LEFT JOIN clinics c ON v.clinic_id = c.id WHERE v.id = ?", [vetId], (err, vet) => {
        if (err) {
            console.error("Error verifying vet:", err.message);
            return res.status(500).json({ error: "Database error verifying veterinarian" });
        }
        if (!vet) {
            return res.status(404).json({ error: "Veterinarian not found" });
        }

        // 2. Fetch specific availability overrides for this date if present
        const availSql = `
        ...
        db.all(availSql, [vetId, date], (err, customShifts) => {
            if (err) {
                console.error("Error fetching availability shifts:", err.message);
                return res.status(500).json({ error: "Failed to fetch availability" });
            }

            // If specific shift entries exist in availability table, use them.
            // Otherwise, apply veterinarian standard daily working shifts.
            let effectiveShifts = (customShifts && customShifts.length > 0) ? customShifts : [
                { start_time: "09:00", end_time: "12:00" },
                { start_time: "14:00", end_time: "17:00" }
            ];

            // 3. Fetch booked appointments for this vet on this specific date (excluding Cancelled)
            let bookedSql = `
                SELECT appointment_time 
                FROM appointments 
                WHERE vet_id = ? AND appointment_date = ? AND status != 'Cancelled'
            `;
            const bookedParams = [vetId, date];
            if (exclude_appointment_id) {
                bookedSql += " AND id != ?";
                bookedParams.push(exclude_appointment_id);
            }

            db.all(bookedSql, bookedParams, (err, bookings) => {
                if (err) {
                ...
                // Determine today's local date and time to filter out past slots for today
                const now = new Date();
                ...
                effectiveShifts.forEach(shift => {
                    ...
                    while (slotMinutes < endMinutes) {
                        ...
                        if (isToday && slotMinutes <= currentTotalMinutes) {
                            isPastTime = true;
                        }

                        if (!isPastTime && !bookedTimes.has(timeStr)) {
                            slots.push(timeStr);
                        }
