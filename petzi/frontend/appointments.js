...
function setupRescheduleModal() {
    ...
    const handleRescheduleDateValidation = () => {
            ...
            if (container) {
                container.innerHTML = `
                    <div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); font-size: 0.85rem; padding: 10px 0;">
                        Select a valid future date to check availability.
                    </div>
                `;
            }
            return false;
    ...
    dateInput.addEventListener('change', () => {
        if (handleRescheduleDateValidation()) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (submitBtn) {
            ...
            submitBtn.innerHTML = `<span class="material-icons animate-spin" style="font-size:1rem; display:inline-block; vertical-align:middle; margin-right:4px;">sync</span> Rescheduling...`;
        }
...
async function fetchRescheduleSlots() {
    ...
    container.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; color: var(--brand-primary); font-size: 0.82rem; padding: 10px 0; font-weight: 500;">
            <span class="material-icons animate-spin" style="font-size: 1.1rem; vertical-align: middle; margin-right: 4px;">sync</span> Checking available slots...
        </div>
    `;
    ...
    try {
        // apiRequest returns parsed JSON directly
        const url = `/api/vets/${selectedRescheduleApp.vet_id}/availability?date=${encodeURIComponent(dateVal)}&exclude_appointment_id=${selectedRescheduleApp.id}`;
        const slots = await window.apiRequest(url);
        container.innerHTML = '';

        if (!Array.isArray(slots) || slots.length === 0) {
            const noSlotsMsg = (dateVal === todayStr) ? 'No available slots for today.' : 'No slots available on this date.';
            container.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; color: #DC2626; font-size: 0.82rem; padding: 10px 0; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 4px;">
                    <span class="material-icons" style="font-size: 1rem;">event_busy</span> ${noSlotsMsg}
                </div>
            `;
            return;
        }
