...
const Auth = {
    ...
    // Logout and redirect to PETZI Home / Landing page
    logout() {
        this.clearAuth();
...
// Ensure pages restored from back/forward cache (bfcache) re-check authentication
window.addEventListener('pageshow', (event) => {
    if (event.persisted && window.Auth) {
        const path = window.location.pathname.toLowerCase();
        const isPublicPage = path.endsWith('home.html') || 
