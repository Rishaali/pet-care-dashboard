...
async function loadPetDetails() {
    ...
    try {
        ...
        petData = myPets.find(p => p.id === petId);
...
async function fetchAvailableSlots() {
    ...
    slotsContainer.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; color: var(--brand-primary); font-size: 0.85rem; padding: 10px 0;">
            <span class="material-icons animate-spin" style="font-size: 1.2rem;">sync</span> Loading available slots...
        </div>
    `;

    try {
        // apiRequest returns parsed JSON directly (a string array of "HH:MM" times)
        const slots = await window.apiRequest('/api/vets/' + vetId + '/availability?date=' + dateVal);
