...
async function loadPets() {
    ...
    try {
        // apiRequest already returns parsed JSON — do NOT call .ok or .json() on it
        myPets = await window.apiRequest(`/api/pets`);
        ...
        myPets.forEach(pet => {
            ...
            option.value = pet.id;
            // Build a clean label using real fields: name — species — breed
            let label = pet.name || 'Unnamed Pet';
            if (pet.species) label += ` — ${pet.species}`;
            if (pet.breed) label += ` — ${pet.breed}`;
        ...
        petSelect.disabled = false;
        petSelect.innerHTML = '<option value="">Unable to load pets — try again</option>';
...
async function performSearch() {
    ...
    try {
        ...
        allVets = await window.apiRequest(`/api/vets/recommendations?disease=${encodeURIComponent(disease)}&city=${encodeURIComponent(city)}&pet_id=${encodeURIComponent(petId)}`);
